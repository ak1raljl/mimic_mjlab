...
class Logger:
    ...
    def init_logging_writer(self) -> None:
        """Initialize the logging writer, which can be either Tensorboard or Neptune and save the code state.

        If the writer is Neptune, the configuration and code state are uploaded as well.
        """
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                ...
                raise ValueError("Logger type not found. Please choose 'neptune' or 'tensorboard'.")
        else:
            self.writer = None
        ...
        if self.writer is not None and self.logger_type == "neptune":
